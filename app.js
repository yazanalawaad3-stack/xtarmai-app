...
// Subtle entrance motion
  ...
  // Quick Insight + Suggested Action (lightweight, no extra backend calls)
  const insightEl = qs('#quickInsightText');
  const suggestedBtn = qs('#suggestedActionBtn');
  const suggestedText = qs('#suggestedActionText');
  ...
  const updateInsight = () => {
    ...
    const todayEl = qs('#todayIncome');
    const netEl = qs('#teamIncome');
    const today = parseAmount(todayEl ? todayEl.textContent : '');
    const net = parseAmount(netEl ? netEl.textContent : '');
    ...
    let msg = '';
    if(today > 0){
      msg = `You earned $${fmtMoney(today)} today · Updated ${hhmm}`;
    }else if(net > 0){
      msg = `Network activity detected · Updated ${hhmm}`;
    }else{
      msg = `No activity yet today · Updated ${hhmm}`;
    }
  ...
  if(suggestedText){
    suggestedText.textContent = 'Open Security Center to protect your account.';
  }

  if(suggestedBtn){
    suggestedBtn.addEventListener('click', () => {
      window.dispatchEvent(new CustomEvent('lux:settings:security'));
      toast('Security Center');
    });
  }
